...
function renderPie(body, widgetId, columns, rows) {
    ...
    legendHtml += '<tr style="border-bottom:1px solid rgba(80,130,255,.15);"><th style="text-align:left;padding:6px 8px;color:#2ee8ff;font-size:10px;">NAME</th><th style="text-align:right;padding:6px 8px;color:#2ee8ff;font-size:10px;">COUNT</th><th style="text-align:right;padding:6px 8px;color:#2ee8ff;font-size:10px;">%</th></tr>';
    rows.forEach((r, i) => {
        const val = r[valueCol] || 0;
        const pct = total ? ((val / total) * 100).toFixed(1) : '0.0';
        const color = PALETTE[i % PALETTE.length];
        legendHtml += `<tr style="border-bottom:1px solid rgba(80,130,255,.06);"><td style="padding:5px 8px;color:#c8dcf8;white-space:nowrap;"><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:${color};margin-right:8px;vertical-align:middle;"></span>${r[labelCol]}</td><td style="text-align:right;padding:5px 8px;color:#e8f0ff;font-weight:600;">${val.toLocaleString()}</td><td style="text-align:right;padding:5px 8px;color:#ffd666;font-weight:700;">${pct}%</td></tr>`;
    });
    legendHtml += '</table>';
    ...
    charts[widgetId] = new Chart(canvas, {
        type: 'doughnut',
        data: { labels: rows.map(r => r[labelCol]), datasets: [{ data: rows.map(r => r[valueCol]), backgroundColor: PALETTE, borderColor: '#060e24', borderWidth: 2 }] },
        options: { responsive: true, maintainAspectRatio: true, cutout: '55%', plugins: { legend: { display: false }, datalabels: { color: '#fff', font: { weight: 'bold', size: 11 }, formatter: (value) => { const pct = total ? ((value / total) * 100).toFixed(1) : '0'; return pct >= 5 ? pct + '%' : ''; } } } }
    });
}
...
function renderBar(body, widgetId, columns, rows) {
    ...
    charts[widgetId] = new Chart(canvas, {
        type: 'bar',
        data: { labels: rows.map(r => r[labelCol]), datasets: [{ label: valueCol, data: rows.map(r => r[valueCol]), backgroundColor: PALETTE.map(c => c + 'cc'), borderColor: PALETTE, borderWidth: 1, borderRadius: 6 }] },
        options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 24 } }, plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'top', color: '#ffd666', font: { weight: '700', size: 11 }, formatter: (v) => v ? v.toLocaleString() : '' } }, scales: { y: { beginAtZero: true }, x: { ticks: { color: '#8daad4', font: { size: 10 } } } } }
    });
}
...
function renderGenericPivot(body, rows, rowKey, headerLabel) {
    ...
    categories.forEach(cat => {
        ...
        dates.forEach(d => { const v = map[`${cat}__${d}`] || 0; colTotals[d] += v; html += `<td>${v > 0 ? v.toLocaleString() : ''}</td>`; });
        html += '</tr>';
    });
...
function renderWidgetBody(widget, columns, rows) {
    ...
    switch (widget.chart_type) {
        ...
        case 'top10_table': renderTop10Table(body, columns, rows); break;
...
function updateMultiDropdown(menuId, btnId, allLabel, optionClass, availableValues, reset) {
    ...
    menu.innerHTML = `
        <li class="px-2"><div class="form-check">
        ...
        ${availableValues.map((v, i) => `<li class="px-2"><div class="form-check">
            <input class="form-check-input ${optionClass} multi-option" type="checkbox" id="${menuId}-${i}" value="${v}" ${currentChecked.has(v) ? 'checked' : ''}>
    ...
    function updateLabel() {
        ...
        btn.childNodes[0].textContent = sel.length === 0 ? allLabel : sel.length === 1 ? sel[0].value : `${sel.length} Selected`;
    }
    updateLabel();

    allCb.addEventListener('change', () => { if (allCb.checked) opts.forEach(c => c.checked = false); updateLabel(); reloadWidgetData(); });
    opts.forEach(cb => { cb.addEventListener('change', () => { if (cb.checked) allCb.checked = false; else if (opts.every(c => !c.checked)) allCb.checked = true; updateLabel(); reloadWidgetData(); }); });
}
...
async function initAllFilters() {
    ...
    regionSel.addEventListener('change', reloadWidgetData);
    ...
    zseSel.addEventListener('change', reloadWidgetData);
    ...
    aseSel.addEventListener('change', reloadWidgetData);
    ...
    if (uidDropdown) {
        ...
        uidDropdown.addEventListener('change', () => {
            ...
            uidInput.value = (val && val !== 'All') ? val : '';
            reloadWidgetData();
        });
    }
